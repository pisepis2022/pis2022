...
def modifyIPersona(request, id):
	...
	IPersona.fechanacimiento=request.POST.get('fechanacimiento',False)
	...
	IPersona.talla=request.POST.get('talla',False)
	IPersona.peso=request.POST.get('peso',False)
	...
	IPersona.tipo=request.POST.get('tipo',False)
...
def getFiliacion(request):
	...
	rFiliacion = Ipolicia.objects.filter(id=idFiliacion, activo="1")
